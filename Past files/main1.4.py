...
def CheckMistakes (string):
    ...
    like no epsilon or omicron with a macron."""
    string += ' '
    len_str = len (string);
    for i in range(len_str):
        if string [i:i + 2] in MISTAKES:
            ErrMsg = Prompts[Prompts['Prompt Name'] == 'error1'].iloc[0]['Prompts'] + '\n' \
            + string [i:i + 2] + Prompts[Prompts['Prompt Name'] == 'error2'].iloc[0]['Prompts'] \
            + string [:i + 3] + Prompts[Prompts['Prompt Name'] == 'error3'].iloc[0]['Prompts'] + '\n' \
            + Prompts[Prompts['Prompt Name'] == 'error4'].iloc[0]['Prompts']
            return ErrMsg
        else:
...
def main():
    appTitle = Prompts[Prompts['Prompt Name'] == 'title'].iloc[0]['Prompts']
    ...
    SettB = wx.Button (MainPanel, -1,
        Prompts[Prompts['Prompt Name'] == 'langButton'].iloc[0]['Prompts'],
    ...
    def HelpPage(event):
        ...
        wx.StaticText(helpPopPanel, pos = (leftBarrier, upperBarrier),
            label = Prompts[Prompts['Prompt Name'] == 'helpMessage'].iloc[0]['Prompts']).Wrap(500 - 30)
        helpPop.Show()

    ycoor += SettB.GetSize()[1] + ypadd
    HelpB = wx.Button(MainPanel, -1,
        Prompts[Prompts['Prompt Name'] == 'helpButton'].iloc[0]['Prompts'],
        pos = (xcoor, ycoor))
    ...
    def AboutPage(event):
        ...
        text1 = wx.StaticText(aboutPopPanel,
            label = Prompts[Prompts['Prompt Name'] == 'aboutMessage'].iloc[0]['Prompts'])
    ...
    AboutB = wx.Button(MainPanel, -1, 
        Prompts[Prompts['Prompt Name'] == 'aboutButton'].iloc[0]['Prompts'],
        pos = (xcoor, ycoor))
...
#   myInput.unbind('<Shift-Return>', None)
    ...
    InterpretB = wx.Button(MainPanel, -1,
       Prompts[Prompts['Prompt Name'] == 'interpretButton'].iloc[0]['Prompts'], pos = (405, upperBarrier))
